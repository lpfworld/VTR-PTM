...
class Transformer(object):
    ...
    def build(
        ...
        **kwargs
        ...
        self.set_inputs(inputs, additional_input_layers, visual_feature_inputs)
        # Other
        ...
        # 多模态输入入口，留作后面使用
        self.visual_features = visual_feature_inputs
        # Call
    ...
    def set_inputs(self, inputs, additional_input_layers=None, visual_feature_inputs=None):
        """设置input和inputs属性
           若是条件bert，则将条件张量依赖的所有输入层一起加到bert的输入层中，构建最终的模型
        """
        if inputs is None:
            inputs = []
        ...
        if visual_feature_inputs is not None:
            if not isinstance(visual_feature_inputs, list):
                visual_feature_inputs = [visual_feature_inputs]
            inputs.extend(visual_feature_inputs)
...
class BERT(Transformer):
    ...
    def apply_embeddings(self, inputs):
        ...
        """
        x, s = inputs[:2]
#         v = self.visual_features
        # z为条件张量（image caption任务下是图片特征）
...
#         v = self.apply(
#             inputs=v,
#             layer=VisualEmbedding,
#             input_dim=2048,
#             output_dim=self.embedding_size,
#             embeddings_initializer=self.initializer,
#             name='Embedding-Visual'
#         )
#         print('token shape :{}'.format(x.shape))
#         print('图片特征{}'.format(v.shape))
        
#         x = self.apply(inputs=[x, v], layer=MergeVisual, name='Embedding-Token-Visual')
#         s = self.apply(inputs=[s, v], layer=MergeVisual, name='Embedding-Segment-Visual')
        ...
        # simplify判断z是否为None，是的话过滤掉z，只留下原始x
        x = self.apply(
            inputs=self.simplify([x, z]),
            layer=LayerNormalization,
            # 条件LN，单层神经网络当作缩放器，适配bert原始LN的向量
            # 1为单层神经元个数， 2为激活函数
