...
# class VisualEmbedding(Layer):
#     """视觉图片Embedding，这里的Embedding是可训练的。
#     """
#     def __init__(
#         self,
#         input_dim,
...
#         **kwargs
#     ):
#         super(VisualEmbedding, self).__init__(**kwargs)
...
#         super(VisualEmbedding, self).build(input_shape)
...
#         """
#         """
#         visual_embeddings = K.dot(inputs, self.embeddings)
...
#         }
#         base_config = super(VisualEmbedding, self).get_config()
    ...
    def build(self, input_shape):
        ...
        if self.conditional:
            ...
            if self.hidden_units is not None:
                # 3层网络结构,VG数据集加入bias加快收敛速度
                self.hidden_dense_0 = Dense(
...
#                 # 3x3网络结构
#                 self.hidden_dense_beta_0 = Dense(
#                     units=self.hidden_units,
#                     activation=self.hidden_activation,
#                     use_bias=False,
#                     kernel_initializer=self.hidden_initializer
#                 )
#                 self.hidden_dense_beta_1 = Dense(
#                     units=self.hidden_units,
#                     activation=self.hidden_activation,
#                     use_bias=False,
#                     kernel_initializer=self.hidden_initializer
...
#                 self.hidden_dense_gamma_0 = Dense(
#                     units=self.hidden_units,
#                     activation=self.hidden_activation,
#                     use_bias=False,
#                     kernel_initializer=self.hidden_initializer
#                 )
#                 self.hidden_dense_gamma_1 = Dense(
#                     units=self.hidden_units,
#                     activation=self.hidden_activation,
#                     use_bias=False,
#                     kernel_initializer=self.hidden_initializer
#                 )               
    ...
    def call(self, inputs):
        ...
        if self.conditional:
            ...
            if self.hidden_units is not None:
#                 # 3
                cond = self.hidden_dense_0(cond)
                cond = self.hidden_dense_1(cond)
            for _ in range(K.ndim(inputs) - K.ndim(cond)):
                cond = K.expand_dims(cond, 1)
                # 3x3
#                 cond_beta = self.hidden_dense_beta_0(cond)
#                 cond_beta = self.hidden_dense_beta_1(cond_beta)
#                 cond_gamma = self.hidden_dense_gamma_0(cond)
#                 cond_gamma = self.hidden_dense_gamma_1(cond_gamma)     
#             for _ in range(K.ndim(inputs) - K.ndim(cond_beta)):
#                 cond_beta = K.expand_dims(cond_beta, 1)
#             for _ in range(K.ndim(inputs) - K.ndim(cond_gamma)):
#                 cond_gamma = K.expand_dims(cond_gamma, 1)
