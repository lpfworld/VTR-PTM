...
parser.add_option("--with_pretrain_weights", dest="with_pretrain_weights", help="pretrain weights.", default=None)
...
parser.add_option("--steps_per_epoch", dest="steps_per_epoch", help="每个epoch需要训练的样本数,总样本数/batch_size得到.", default=180000)
...
pretrain_model_weights_path = options.with_pretrain_weights
...
def read_object_data(folder, valid=False):
    ...
    -valid:
    [{'image_id':str,
      'regions': [{'region_feature'：[2048], keywords': str, 'caption': str}, ...],
    ...  
    ]
    """
    print('-Read data ...')
